              ...
              )
{
  ...
  switch (_type)
  {
    ...
    case TESSELATION : { m_shaderHandle =0; std::cerr<<"not yet implemented\n"; }
  }
  m_compiled = false;
  m_refCount=0;
  m_source=0;
                           ...
                           )
{
  ...
  // horrible coercion to make modern C++ work with C!
  const char *data = _string.c_str();
  glShaderSource(m_shaderHandle , 1, &data,NULL);
  m_compiled=false;
  // we need this for the chek in the compile bit
